...
import imageio # For reading and writing images and creating GIF animations
import os # For file and directory management

# Average Plots

# Load summary data containing averages and standard deviations for each beta
df = pd.read_csv("Results_1x100/ising_averaged_results_1x100.csv")
...
dist_df = pd.read_csv("Results_1x100/ising_all_runs_1x100.csv")
...
energy_csv = open("Results_1x100/energy_distribution_summary_1x100.csv", "w", newline='')
mag_csv = open("Results_1x100/magnetization_distribution_summary_1x100.csv", "w", newline='')
...
print("  - Results_1x100/energy_distribution_summary_1x100.csv")
print("  - Results_1x100/magnetization_distribution_summary_1x100.csv")

# Read all saved frame CSVs and create PNGs
frames = []
file_list = sorted(os.listdir("Results_1x100/frames_1x100"), key=lambda x: int(x.split("_")[1].split(".")[0]))

for file in file_list:
    data = pd.read_csv(f"Results_1x100/frames_1x100/{file}", header=None)
    fig, ax = plt.subplots(figsize=(6, 6))
    ax.imshow(data, cmap='coolwarm', vmin=-1, vmax=1)
    ax.axis('off')
    step_num = file.split("_")[1].split(".")[0]
    step_size = 1000000 // 5  # Adjust if needed
    plt.title(f"Step {int(step_num) * step_size}", fontsize=12)
    frame_path = f"Results_1x100/frames_png_1x100/{file.replace('.csv', '.png')}"
    plt.savefig(frame_path)
    plt.close()
    frames.append(imageio.imread(frame_path))

# Create animated GIF
imageio.mimsave("Results_1x100/ising_simulation_1x100.gif", frames, duration=0.9)
print("GIF saved as Results_1x100/ising_simulation_1x100.gif")
